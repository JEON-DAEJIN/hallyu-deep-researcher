...
> **자기 점검 — "나란히 읽기"는 여기서 절반만 했다**: N043 12장은 "같은 질문으로 보고서 여러 편
> (기본·장치 하나 끈 것·혼자 하는 대조군)을 만들고, 나란히 직접 읽어 어느 쪽이 나은지와 그 이유를
> 자기 말로 적으라"고 요구한다. 아래 "공정한 대조군 비교"에서는 팀 vs 대조군 실제 보고서를 손으로
> 읽고 오귀속 사례까지 짚었지만(§3도 참고), **ablation 30회는 지표(근거율·편중 등) 비교 위주로만
> 처리했다** — `ablation.py`가 각 설정의 보고서 원문을 저장하지 않고 `output/ablation.json`에
> 지표만 남기기 때문이다. 즉 "장치 하나 끈 보고서"를 실제로 나란히 읽는 절반은 이번엔 생략됐다.
> 치명적 결함은 아니지만(수치 비교 자체는 30회 반복으로 뒷받침됨), 12장 요구를 완전히 충족한 것은
> 아니라는 점을 스스로 기록해둔다. 보완하려면 `ablation.py`가 설정별 최종 보고서 텍스트도
> `output/reports/ablation_<설정>_<질문>_<회차>.md`로 저장하도록 고치면 된다.
...
| 평가기준 | 이 보고서에서 답한 곳 |
|---|---|
| 성능 검증 및 실패 분석 | "가장 공들인 부분"(4건의 실제 버그와 추적 과정), "공정한 대조군 비교"(질문4를 직접 읽고 어디서 차이가 났는지 서술). 단, ablation 30회는 지표 비교 위주였고 설정별 보고서를 나란히 읽는 절반은 생략했다는 자기 점검을 "Ablation 결과" 절에 명시 |
...
- **Ablation 보고서 나란히 읽기**: 위 "Ablation 결과" 절의 자기 점검대로, `ablation.py`가 설정별
  최종 보고서 텍스트를 저장하도록 고쳐서 기본/배정 끄기/역할 끄기 등을 실제로 나란히 읽고 비교하는
  절차를 추가하고 싶다. 지금은 지표만 비교했다.
